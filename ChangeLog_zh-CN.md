...
- [x] `#240915` cli.Option 执行 map 类型，gen 亦支持 map 解析
- [x] `#240924` cmd/uymas2 正式发布后应更名，使其不与 v1 版的 uymas 混淆
- [x] `#241019` 系统常量或函数等加入关键的文档注释，使其不忘记具体作用或来源原有
...
- [x] `#250901` `data/input` 转数字时，支持 k(Kilo)，w(万)，m(mega)，b(billion)等数值单位，如`1.5m -> 1500_000`
- [ ] `#290416` `cli` 包，*StructGen* 优化，支持 Array等多种类型（通过 Option 的键值去映射struct的字段存在问题，不一定匹配的上）
...
- **cli**
  - pref:  使`cli`及 `arg` 的配置项优化，使其不设置是统一启用默认值
  - pref: 优化 `StructGen` 下长短选项的二级键值获取
...
> 程序优化，根据 idea 2025.2 版本提示优化代码格式化
...
- **rock**
  ...
  - feat: 新增函数 `ListHasIntersect` 用来检查列表是否存在检查列表中的一个
- **logger**
